...
<objective>
Integrate SearchBar into site navigation, import all posts to Algolia, and verify complete search functionality.

Purpose: Complete the search feature by connecting the SearchBar to the navigation and populating the Algolia index with all published posts.
Output: Working typeahead search accessible from every page on the site.
</objective>
...
- Post model has Searchable trait
...
4. Typing in search bar shows results from Algolia
5. Typo tolerance works (misspellings still find results)
6. Clicking result navigates to post URL
