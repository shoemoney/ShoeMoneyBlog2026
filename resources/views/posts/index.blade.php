<x-layout>
    ...
    @php
        ...
        $heroAwardText = \App\Models\Setting::getValue('hero_award_text', '');
        $heroAwardUrl = \App\Models\Setting::getValue('hero_award_url', '');
    ...
    @if($heroPhoto || $heroBlurb)
        <section class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 mb-10 shadow-sm overflow-hidden">
            {{-- Award Banner --}}
            @if($heroAwardText)
                <div class="bg-gray-100 dark:bg-gray-700/60 px-4 py-2.5">
                    <div class="flex items-center justify-center gap-2 text-sm font-semibold text-blue-600 dark:text-blue-400">
                        <svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                        @if($heroAwardUrl)
                            <a href="{{ $heroAwardUrl }}" target="_blank" rel="noopener"
                               class="hover:text-blue-800 dark:hover:text-blue-300 transition-colors">
                                {{ $heroAwardText }}
                            </a>
                        @else
                            <span>{{ $heroAwardText }}</span>
                        @endif
                        <svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 font-display">Reader Favorites</h2>
            </div>
