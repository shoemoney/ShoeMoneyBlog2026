<x-layout>
    <article>
        <header class="mb-8">
            ...
            <div class="mt-4 flex items-center text-gray-600 dark:text-gray-400 space-x-4">
            ...
            @if($post->categories->isNotEmpty())
                <div class="mt-4 flex flex-wrap gap-2">
                    @foreach($post->categories as $category)
                        <a href="{{ $category->url }}"
                           class="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-sm rounded-full hover:bg-blue-200 dark:hover:bg-blue-800">
                            {{ $category->name }}
                        </a>
                    @endforeach
                </div>
        ...
        @if($post->tags->isNotEmpty())
            <footer class="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700">
                <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tags</h3>
